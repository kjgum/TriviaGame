...
function renderQuiz() {
  for(i=0;i<questions.length;i++) {
    ...
    for(j=0;j<choices[i].length;j++) {
      document.writeln('<input type="radio" name="answer_' + i + '" value="' + choices[i][j] + 
        '" id="answer_' + i + '_' + j + '" class="question_' + i + '" onclick="submitAnswer(' + i + 
        ', this, \'question_' + i + '\', \'label_' + i + '_' + j + '\')" /><label id="label' + i + '_' 
        + j + '" for="answer_' + i + '_' + j + '"> ' + "  " + choices[i][j] + '</label><br />');
    }
  }
